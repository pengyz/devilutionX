...
**日期**：2026-08-08
**状态**：已实施（2026-08-08，独立复核通过）
**分类**：Depth（2026-08-10 定位变更后并入 Expansion 层，见宪章决策 31）
**取代**：无（新规格）

> **2026-08-10 定位变更补注（宪章决策 31）**：本文档写作于「深度层开关」语境。2026-08-10 项目定位变更为「内容与机制扩充的中大幅度改进版 Mod」，**移除全部开关，所有改动默认开启**。本文档涉及开关的段落（第 2 节判定、4.1 开关实现、4.7 开关关闭行为、红线 13、验收标准）按以下方式解读：Dark Expedition 无条件生效，`darkExpedition` 选项已从代码删除，`IsDarkExpedition()` 语义恒真。红线 13（开关关闭回退原版）已从宪章删除，其位置由新的红线 13（与既有系统内生）取代。
...
**归类结论**：整个改动包（M1' 视野限制 + 反制体系 + stub 清理）作为一个整体判为 **Depth**（定位变更后为 **Expansion**），由单一深度层开关控制（定位变更后无条件生效，开关已移除）。
...
**已实施**。2026-08-08，独立复核（Oracle）通过。670/670 测试全过，漂移校验 5 项 PASS，timedemo 存档比对与本分支基线一致。

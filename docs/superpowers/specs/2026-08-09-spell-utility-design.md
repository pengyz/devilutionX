...
**日期**：2026-08-09
**状态**：已实施（2026-08-09，两轮规格复核 + 实施独立复核通过）
**分类**：Depth（2026-08-10 定位变更后并入 Expansion 层，见宪章决策 31）
**取代**：`archive/specs/spell-system.md`（已废弃，含已核实错误）

> **2026-08-10 定位变更补注（宪章决策 31）**：本文档的「Depth / 红线 13（开关关闭）」表述作废。Etherealize 删除在任何开关状态下均不可达（原版即无获取渠道），定位变更后无条件生效，行为无差异。分类并入 Expansion。

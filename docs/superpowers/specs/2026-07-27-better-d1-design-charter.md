...
结论：**宪章必须由可判定命题构成。** 需要品味判断的地方只允许有一处——判断一个改动是否值得进入扩充层——其余全部靠查代码或查文档回答。
...
> Better D1 是面向 D1 老玩家的 DevilutionX mod，属于对游戏内容与机制进行扩充的中大幅度改进版：底座层让 D1 更易读、更少摩擦，且不改变任何平衡；扩充层用新增内容与机制修改（而非膨胀数值）的方式丰富游戏体验，默认开启，不提供开关。
...
| | 底座层 Base | 扩充层 Expansion | 基础设施 Infra |
|---|---|---|---|
| 默认 | 开启，不可关 | 开启，无开关 | 始终生效，不受任何开关影响 |
| 目标 | 消除摩擦 | 扩充内容与修改机制 | 降低未来改动成本 |
| 硬约束 | 不改变任何平衡 | 不膨胀数值（但允许新增内容与修改机制） | 玩家可感知行为零变化 |
| 受众 | 全部老玩家 | 全部老玩家（安装即生效） | 作者与 mod 作者 |

Base 与 Expansion 是层（影响玩家），Infra 不是层（不影响玩家），因此 Infra 不受无开关原则约束。

### 无开关原则

本项目定位为「对游戏内容和机制进行扩充的中大幅度改进版 Mod」。因此：

**不提供任何开关，所有改动默认开启。** 玩家安装 Better D1 即选择了这些改动，不需要（也不提供）逐项关闭的入口。

理由：逐特性开关产生 2^n 种未测试的组合；而且「全都做成选项」是逃避设计决策——如果某一项必须单独关掉才能玩，那说明那一项设计错了，应该修它，不是给它一个开关。这与原宪章深度层开关的论证一致，只是把「默认关闭」翻转为「默认开启」：定位从「可选的黑暗模式」改为「整体改造后的 D1」。

Dark Expedition 作为扩充层的一部分无条件生效：视野限制、CanTarget、Infravision 卷轴预算、掉落排除全部常开。代码中不保留 `darkExpedition` 选项。
...
一个已知的不舒服后果：取消符文和伤害卷轴的掉落是纯减负（这两类物品几乎无人使用），但它改变掉落表构成，因此属于 Expansion。它无条件生效——既然不再有开关，掉落排除就是 D1 的常态。
...
扩充层支柱编号加前缀 `DP`，以免与游戏名 D1 混淆。

**DP1 · 用新增内容与机制修改制造决策，不用膨胀数值**

想让游戏更丰富，手段是新增内容（新怪物/新物品/新机制）带来的新决策，不是给既有怪物加生命、给既有掉落加数值。数字膨胀是偷懒的「丰富」——它没有制造任何新选择。

此条允许的内容类型：
- **新内容**：新怪物（带独特行为或掉落）、新独特物品（带独特机制）、新法术/技能书、新任务、新层段主题
- **新机制**：与既有系统互动的机制修改（如资源预算、信息揭示、走位约束），必须与 D1 既有心智模型内生（禁令 4）

**DP2 · 每个新增内容都要有反制与明确作用**

玩家无法应对的新增不是内容，是税。新怪物必须有明确的应对方式（指名具体装备、消耗品或行为）；新物品必须有明确的使用场景；新机制必须有反制手段。反制手段必须指名具体装备、消耗品或行为。

**DP3 · 新增内容产生取舍，不产生负担**

新增内容必须转化为「选这个还是那个」的取舍，不是「多背一条规则」或「多跑一趟腿」。如果一项新内容不产生任何取舍，它就是在增加噪声——删除它而不是保留它。

**DP4 · 与既有系统内生**

新内容/新机制必须与 D1 既有机制产生互动，而不是平行存在。判定问法：玩家在 D1 中学到的既有知识（走位、资源预算、信息稀缺）能帮助他理解这项新内容吗？如果答案是「不能，这是全新的另一套规则」，它违反了禁令 4 的「不新增玩家必须记住的概念」。
...
| # | 判定 | 必须 |
|---|---|---|
| 1 | 按第 2 节判定树输出 Base、Expansion 或 Infra | 已判定 |
| 2 | 问题陈述是否指向具体症状（可复现的操作序列，或具体数值 + 出处），而不是「感觉不够深」？ | 是 |
| 3 | 文档引用的每个数值是否标注出处（文件路径 + 字段名）？ | 是 |
| 4 | 标注为「已实施」的每个函数是否有非测试调用者？**以及验收标准是否全部通过？** | 是 |
...
| # | 判定 | 必须 |
|---|---|---|
| 9 | 该改动是新增内容/机制修改，还是膨胀数值？ | 新增内容或机制修改 |
| 10 | 每个新增内容有可执行的反制与明确作用吗？必须指名具体装备/消耗品/行为 | 有且已指名 |
| 11 | 新增内容产生的是「选这个还是那个」的取舍，还是「多记一条规则/多跑一趟」？ | 取舍 |
| 12 | 该改动在全部相关层段（1–16 / 17–20 Nest / 21–24 Crypt）都有定义吗？ | 是 |
| 13 | 该改动与既有系统产生互动吗（还是平行存在的另一套规则）？ | 是 |
| 14 | 该改动对近战与远程职业的影响是否已分别评估？ | 已评估 |

红线 14 由光照压制的失败推出：视野从 10 缩到 5–6，近战玩家几乎无感（本就要贴到 1 格），远程玩家的核心优势被砍掉。任何改动都天然可能对职业影响不均，必须显式评估，否则会变成一次无意的职业平衡改动。
...
| 分类判定 | Base、Expansion 或 Infra，附第 2 节判定树的逐步判定结果 |
| 事实基础 | 引用的每个数值一行，格式见下 |
| 方案 | — |
...
| 改动 | 出处 | 判定 | 裁决 |
|---|---|---|---|
 | 金币掉落 +25% | `Source/items.cpp:3178` `rndv = rndv * 5 / 4` | Expansion（规则 2） | **撤销**。经济重构留待待立项的消耗品经济规格 |
 | 职业差异化堆叠上限 | `Source/items.cpp:5146` `GetMaxStackCount` | Expansion（规则 6） | **撤销加成**，统一药水 5 / 卷轴 3。另违反扩充层红线 11 |
 | 腰带消耗品堆叠 | `Source/items.h:258` `_iStackCount`；`Source/inv.cpp` 腰带逻辑 | Expansion（规则 6） | **改为背包堆叠、腰带不堆叠**（待立项）。理由见下 |
 | 光照压制 | `Source/lighting.cpp:594`、`:620` | Expansion（规则 4） | **撤销**。范围见下 |
...
且其当前设计已判定不满足 DP2（无反制手段）与 DP3（不产生预算取舍），并违反新增的扩充层红线 14（对远程职业的削弱远大于近战）。

**2026-08-10 补注（决策 31 定位变更）**：撤销时的顾虑「作为 Depth 重新引入时代码大概率要重写」已被 Dark Expedition 印证——Dark Expedition 以满足 DP2/DP3 的方式重新实现了视野限制（Infravision 卷轴预算作为反制、CanTarget 修复远程劣势），代码确系重写而非复用。定位变更后 Dark Expedition 无条件生效，光照压制确定不会被原样复活：它的设计缺陷（无反制、无预算取舍、职业影响不均）在无开关语境下同样致命。
...
| 待立项 | 分类 | 优先级 | 依赖 | 参考文件 |
|---|---|---|---|---|
| ~~深度层旗舰改动（方向待定）~~ | Expansion | 1 | — | — |
| 消耗品经济重构：取消符文及伤害卷轴掉落（**已完成**，见下），重新推导补偿形状，**并一并裁决消耗品堆叠的最终形态**（见决策 26） | Expansion | 3 | — | `archive/specs/consumable-system.md`、`archive/specs/2026-07-07-consumable-stacking-design.md` |
| 法术实用性：Rage / Etherealize / Golem（**已实施**，见 `2026-08-09-spell-utility-design.md`） | Expansion | 4 | — | `archive/specs/spell-system.md` |
| 内容扩充方向（新怪物/新物品/新机制候选）——决策 31 定位变更后开放的立项类别 | Expansion | — | — | 见宪章第 11 节决策 31 |
| **密度修复框架（2026-08-10 确立）**：修复怪物行为密度塌缩（洞穴远程垄断 38% / 地狱多样性骤降 / 教堂无压力）。已确立 6 份规格：A1 教堂骷髅区分（v3）、A3 洞穴风筝组合（v3）、B1 采样反垄断（v3）、C2 熄灯者（v4）、A2 激励者（v3）、E1 地狱悬崖评估（可定稿）。经两轮 Oracle 独立复核 + 整体验证（9 项引擎事实核实）。实现前需按依赖序（harness → A1 → A3 → B1 → C2 → A2）立项 | Expansion | — | 整体验证已完成 | `2026-08-10-density-fix-framework-overview.md`、各规格 |
...
**扩充层旗舰「黑暗远征」（Dark Expedition）已于 2026-08-08 实施**，见 `2026-08-08-dark-expedition-design.md` 与 `plans/2026-08-08-dark-expedition.md`。完整落地：视野限制（Hell 60%/Nest 85%/Crypt 50%）、Infravision 目标选择修复（`CanTarget`）、Infravision 卷轴稀缺预算、掉落排除（符文/伤害卷轴）、三个 stub 法术删除。经两轮 Oracle 对抗评审 + 一轮独立复核；670 项测试全过，漂移校验 5 项 PASS，CI 全绿。

**2026-08-10 定位变更（决策 31）**：Dark Expedition 不再由 `darkExpedition` 选项门控，改为无条件生效——代码删除该选项，`IsDarkExpedition()` 语义恒真，相应测试断言改为无开关形式。扩充层从「默认关闭的可选黑暗模式」变为「默认开启的整体改造」。
...
> 历史语境（2026-07-27 决策 28 分析，已过期）：此段写于深度层为空、旗舰方向未定之时。Dark Expedition 已于 2026-08-08 实施解决此焦虑；2026-08-10 定位变更（决策 31）后深度层并入扩充层（Expansion），无开关、默认开启。
...
| 31 | **定位变更**：从「底座层 + 默认关闭的深度层」改为「对游戏内容和机制进行扩充的中大幅度改进版 Mod」。具体：**(a)** 移除深度层开关，不提供任何开关，所有改动默认开启；**(b)** 深度层更名为扩充层（Expansion），手段从「限制信息」放宽为「新增内容 + 机制修改」（仍禁止数值膨胀）；**(c)** 支柱 DP1 放宽为「用新增内容与机制修改制造决策，不用膨胀数值」，新增 DP4「与既有系统内生」；**(d)** 红线 13（开关关闭回退原版）删除，红线 9 改为「新增内容/机制修改 vs 膨胀数值」，红线 10/11 扩展至内容扩充语境，红线 12 保留、14 保留；**(e)** Dark Expedition 移除 `darkExpedition` 选项，无条件生效；**(f)** 新增待立项类别「内容扩充方向」。动机：D1 内容量少是核心问题，项目转向「尊重原内核的前提下增加游戏内容与机制」；「限制信息」的收益有下限（视野已到 0.5× 接近地板），内容多样化的收益没有 | 已定 |
| 32 | **密度修复框架确立**（决策 31 后首个内容扩充立项）：系统性扫描确认密度塌缩（洞穴远程垄断 38% / 地狱多样性骤降 + 数值悬崖 / 教堂无压力）。经 Oracle 框架评审 + 独立复核收敛为最小连贯包，再经两轮独立复核 + 整体验证（9 项引擎事实核实）后确立 6 份规格：A1 教堂骷髅区分（v3，冲锋改即时）、A3 洞穴风筝组合（v3，即时冲锋+冷却）、B1 采样反垄断（v3，Counselor 映射修正后 L15 同类尾 27.8%）、C2 熄灯者（v4，环境趣味，Infravision 反制降级）、A2 激励者（v3，REJECT 后重构：普通散群无 leader，改位置半径光环）、E1 地狱悬崖评估（REJECT 后重构：GetMinHit 钳制证明数值悬崖非真实问题，不降数值，可定稿）。**根因教训**：规格引用引擎机制必须核实代码（虚构冲锋前摇/误读群落系统/虚构 Infravision 通道/误分类 Counselor 均为未核实导致）。全部规格定稿，实现前按依赖序（harness → A1 → A3 → B1 → C2 → A2）立项 | 已定 |
| 33 | **定位再澄清（2026-09-15）**：本项目是**大型修改**（而非 QoL mod / 小型扩充），目标是深度改造游戏体验。对"是否颠覆 D1 核心体验"暂不决定：先做 P0（采样与编组的实测与改造）拿到核心数据，评估"不颠覆能走多远"是否符合预期；若不达预期，**不排除颠覆性改造**。本决策细化决策 31 的"中大幅度改进版"措辞 | 已定 |
| 34 | **不保证存档兼容性，甚至不保证存档格式兼容（2026-09-15，作者明确授权）**。解锁：①`Monster`/`Item`/`PlayerPack` 等结构可自由增删字段与调整语义（`monster.levelType` 作为 `LevelMonsterTypes` 索引的重映射代价被接受）；②E3 的 L1「靠 `PlayerPack` 保留位零格式破坏」不再独有优势，L2 的 `ItemPack` 19B 无保留位也不再是硬阻塞；③新增持久化字段不必为旧存档做映射。**保留的纪律（不是兼容性而是可诊断性）**：任何格式改动必须留**版本标记**，版本不匹配时**明确拒绝或明确忽略该存档**，**不得静默误读**——仓库已有两起静默数据损坏事故（`docs/knowledge/gotcha_save_bid_overwrite.md`、`gotcha_save_stack_append.md`）；多人要求同版本；硬编码测试夹具（`pack_test` 期望数组、`writehero` golden SHA、`timedemo` 参考存档）须按新确定性重新生成，不得靠手改数字 | 已定 |
| 35 | **存档兼容性分期（2026-09-15，细化决策 34）**：**初版功能阶段不做任何存档兼容工作**——不设版本迁移、不为旧存档做映射、不为字段增删做兼容路径。**兼容层在初版功能完成后单独立项**。现阶段唯一要求：① 每次改动格式后**重新生成**硬编码夹具（`pack_test` 期望数组、`writehero` golden SHA、`timedemo` 参考存档、采样用例），不得手改数字；② 在 `docs/knowledge/decision_save_format_policy.md` 的**格式变更台账**记一行（给将来的兼容层留地图）；③ 多人要求同版本。理由：开发期无外部玩家，旧存档可直接丢弃；把兼容成本推迟到格式稳定之后，避免为一批很快会再变的格式做迁移 | 已定 |

...
| **L16 单列** ⚠ | **该层无 squad 参数行** ⇒ 小队机制被绕过 ✗。**两种处置（二选一，必须与守卫同一次原子改动）**：**(i) 补 L16 的 params 行**（推荐 ✓ —— 消除不对称，且守卫改为"每层含 L16 都有参数且比率达标"✓）；**(ii) 保持缺失**（守卫则必须保留 `EXPECT_EQ(GetLevelRosterParams(16), nullptr)` ✓，并接受该层无小队形态 ✓） | 附录 G.1 ✓ |
...
| (f) | **按层小队比率**：`rolls / max(1, eligibleCoreDraws) ≥ 0.30 - 0.05`（`GetSquadRollStats()` ✓，每层 N=50-200 次建关取均值 ✓） | 把某层 `squad_chance` 由 30 改成 5 ✓（**既有 fixture `level_roster_params_squads_*.tsv`** 可直接用于反证 ✓） |
| (f2) | **L16 二选一**：补行 ⇒ 纳入 (f) ✓；不补 ⇒ `EXPECT_EQ(GetLevelRosterParams(16), nullptr)` ✓ | 偷偷给 L16 补行（会让该层真的出小队 ✗）✓ ／ 删掉该断言 ✓ |
...
6. **L16 的处置（v3 新增，必须你定）**：**(i) 补 L16 的 squad 参数行**（推荐 ✓，但要与守卫同一次原子改动 ✓）／**(ii) 保持缺失**（则保留 `GetLevelRosterParams(16)==nullptr` 断言，并接受该层没有小队形态 ✓）。

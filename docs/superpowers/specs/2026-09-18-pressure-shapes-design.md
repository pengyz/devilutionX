...
**状态**：**已批准**（2026-09-18 作者"按照建议来"：§8.1 六问全部按建议通过 ✓）→ **实施中**（待写实施计划 ✓）
...
#### 4.2b 形态的落实方式（v3，由附录 E/F/G 的基线与复核结论限定）

| 形态 | 落实方式（**全部调既有参数/放置**，不新建机制 ✓） | 依据 |
|---|---|---|
| L1-8 可逃的围压 | 维持低 `squad_chance/size`（现状 30/2 ✓），**保证通路可绕**（放置策略断言 ✓）；**首层 L1 无精英** ✓ 符合教学定位 | 附录 G.1/G.2 ✓ |
| L9-12 狭窄压制 | 小队 + **构成**把远程/施法者放到通路（名册 ✓）；现状该段类型最丰富（4 类 ✓） | 附录 E.2 ✓ |
| L13-16 消耗战 | 提高小队出现率/规模（现状仍 30/2 ✗ 未塑形）⇒ 连续交战 | 附录 G.1 ✓ |
| **L17-24 精英抉择** | **以拴系小队为主** ✓：**L21-L23 无任何精英** ✓ ⇒ 只能靠小队；**L19/L20/L24 可复用既有 boss** ✓（但那要动 `GetLevelMTypes()` 的硬编码 ✗ 属额外工作量） | 附录 G.2 ✓ |
| **L16 ⚠ 待改判** | **前提被推翻** ✗：`GetLevelMTypes()` 对 `currlevel==16` 早退（`:3561-3566` ✓）⇒ **补参数行不会产出小队** ✗ ⇒ 选项见 §8.1 第 6 条（**我推荐 (a) 不补行** ✓） | 附录 G.1 ✓ |

> **注意**：§4.2 原表里"L17-24 含精英小队""L13-16 近远交替"等表述，**以本小节为准** ✓（原文是设计意图，本节是经实测限定后的落实方式 ✓）。
...
### 6.1b 守卫落地清单（v3：整合附录 E / F.3 / G 的结论；**每条都必须能"改坏即红"** ✓）

| # | 断言 | 改坏什么会红 |
|---|---|---|
| (a) | `types(seg) == 黄金集`（各层段需求类型集合等值） | 增/删任一类型 ✓ |
| (b) | `AIs(该层段名册 core) ⊆ AIs(导出映射)`（附录 E.1/E.1b ✓） | 新 AI 进名册 ✓ |
| (c) | **发射点白名单**：正则扫 `AddMissile(` / `StartRangedAttack(` / `StartRangedSpecialAttack(` 的**字面 `MissileID` 实参**，与白名单等值 | **新增硬编码施法** ✓ |
| (d) | 前提：**不存在 `ACIDRES` 族** ✓，且**反制池内的族名不得是 `*_CURSE`** ✓（**修正 v4**：原文"族 ∩ `{*_CURSE}=∅`"为假 ✗——源表本就含 14 个 `*_CURSE` 族（含 `LIGHT_CURSE`）✓，它们只是**不是反制**；判据已改为"咒族由数据推导、池内不得含咒族"，且断言**咒族非空**以防解析退化 ✓） | 有人加酸抗 / 把某个咒族写进反制池 / 表结构变化致咒族解析为空 ✓ |
...
**实现偏离（v5，2026-09-18 复核后记录）**：①实际用**双侧** `EXPECT_NEAR(rate, expected, 0.05)`（**比本行单侧更严** ✓，可抓"率异常偏高" ✓）；②黄金值落在**夹具 `test/fixtures/pressure/squad_rates.txt`**（非本行曾写的 `ExpectedSquadRateForLevel()` 代码函数 ✓）；③**已实测范围＝L1-15**（L16 无参数行 ✓；**L17-24 缺 HF 素材未测** ⚠，见计划任务 2 段 ✓）。
...
**前置基线状态**：附录 C（密度/类型）✓、附录 E（类型覆盖与发射点）✓、附录 G（小队参数与分布）✓ **已取**；
**仍缺** ⚠：真实"**遭遇同时出现 ≥2 种需求类型的比例**"（需按附录 G.4b 的骨架扩展一次测试 ✓）。
...
### 8.1 审计问题（**已由作者决定**，2026-09-18「按照建议来」✓）

1. **决定：地狱段"何时回城"这一条决策，改到 C 腿（补给成本）之后再验收** ✓（现状回城便宜 ⇒ 现在不成立 ⚠）；
2. **决定：四种形态照 §4.2b 执行** ✓（教堂段低概率 1-2 队 ✓；L17-24 以拴系小队为主 ✓）；
3. **决定：试玩问法照准** ✓（准备/撤回项 + **反跑腿项**（偏向新版即撤销）✓）；
4. **决定：保留 (g) 逐字节不变量断言** ✓（作为我"不越界"的硬约束；合法数据改动需**显式更新**黄金哈希 ✓）；
5. **决定：基线测量规模 N = 100** ✓（每层 100 次建关取均值；仓内已有 500 seeds/层先例 ✓，若套件变慢可下调并记录 ✓）；
6. **L16 采用 (a)（不补行）** ✓ —— **作者于 2026-09-18 正式确认**（原为默认生效 ✓）（2026-09-18 作者"认可，继续"＝保持默认 ✓；此前"前提被推翻"的改判说明保留在下 ⚠）（默认 (a) 已体现在守卫与测试中 ✓）
   **原改判依据（保留）**：：**`GetLevelMTypes()` 对 `currlevel == 16` 早退** ✓
   （`Source/monster.cpp:3561-3566`：只注册 `MT_ADVOCATE`/`MT_RBLACK`/`MT_DIABLO` 后 `return {}` ✓）⇒ 名册路径被绕过，
   且**没有第二条已注册的 core 可配对**（`PickCorePartnerTypeIndex` `:3375`）⇒ **给 L16 补参数行"实际不会产出小队"** ✗✗
   （该行的 `max_image/tail_draw/class_floors` 在该层也全是**死数据** ✓）。
   ⇒ 我先前"补行＝消除不对称并让 L16 有形态"的说法**是错的** ✗，故原决定 (i) 的依据不成立 ✓。**三个选项（请裁决）**：
   **(a) 不补行** ✓（最小、诚实 ✓：保留 `EXPECT_EQ(GetLevelRosterParams(16), nullptr)` ✓，并接受 **L16 没有小队形态** ✓——该层的压力只能来自 `.dun` 嵌怪与既有机制 ✓）；**（我改推荐此项 ✓）**
   **(b) 补行 + 同时让 L16 走名册路径**（须改 `GetLevelMTypes()` 的早退 ✗ ⇒ 会**改变 L16 的怪物构成**，属设计变更，需另过形态论证与前/远程评估 ✓）；
   **(c) 补行但仅供文档对称** ✗（**死数据** ✗，不推荐 ✓）。
...
**L1/L5 裁决（2026-09-18 ✓）**：作者**认可默认 (a)「接受并记录」** ✓ —— **不改** L1/L5 的构成 ✗（不在本腿引入"首层教学双类型"这一设计变更 ✓）；仪器与黄金表**如实保留**两个低值 ✓（`L1 = 0.00` ✗、`L5 = 0.18` ⚠），若日后要做另立"**L1 构成变更**"项（须先过契约复测与近/远程评估 ✓）。

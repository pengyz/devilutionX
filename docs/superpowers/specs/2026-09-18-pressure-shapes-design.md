...
| **L16（已决 ✓）** | **补 L16 的 squad 参数行** ✓（作者决定 (i)）⇒ 该层纳入小队形态；**与守卫改动同一次原子提交** ✓（守卫改为"含 L16 每层都有参数行且比率达标" ✓） | 附录 G.1 ✓ |
...
| (f2) | **L16 参数行存在且被 (f) 覆盖**（作者决定补行 ✓）：`ASSERT_NE(GetLevelRosterParams(16), nullptr)` ✓ | 有人**删掉 L16 的参数行**（该层会静默失去小队形态 ✗）✓ |
...
6. **决定：L16 采用 (i) 补 squad 参数行** ✓ —— 消除不对称，且守卫改为"**含 L16 在内的每层都有参数行且小队比率达标**" ✓；
   **原子性要求**：补行与守卫改动必须在**同一次提交**内完成 ✓（否则期间守卫处于错误状态 ✗）。

...
**作者裁决：选 (c) 扩池**（2026-09-18）。落地方式与实测：

| 步骤 | 内容 | 结果 |
|---|---|---|
| ① 窗口 | `mods/hf/.../monstdat.tsv`：`MT_STINGER`、`MT_FELLTWIN` 的 `maxDunLvl` 18 → **20**（两只都是 SkeletonMelee = **非施法者**；它们早已是 L17 的合法名册行 ⇒ 非 unique base ✓） | L19/L20 各多一个非施法者候选 |
| ② core | **L19** = `{VENMTAIL, STINGER, TORCHANT}`（1 施法者）；**L20** = `{FELLTWIN, LASHWORM}`（0 施法者） | B1 3/2 ✓、**B3 = 0.0** ✓ |
| ③ 实测 | 远程占比 L19 **0.039114**、L20 **0.0646188**（≤vanilla 0.1210/0.1869）；施法者 L19 **0.301562**、L20 **0.198201**（≤ceiling）；组合数 **4 / 5**；并集 **10 / 9** | 三条 HF 守卫 **3/3**、地板/unique/B2 **3/3**、校验 41/41 ✓ |

**(c) 严格优于 (a)(b)**：它修好 B3 的同时**提升内容与多样性并压低两项压力**，没有放宽任何阈值 ✓。

**三条出路（已由作者裁决，见上；此处保留原始选项记录以便复盘）**：

...
- [x] **步骤 2：L1-8 可绕性 —— 撤下、交由试玩（作者裁决 (b) ✓，2026-09-18）**

**停手理由（按计划自身规则 ✓）**：测试/生产两侧**均无可达性判据** ✗（`IsTileSolid -> dPiece -> pMegaTiles` 链只有注释 ✓，无 BFS/连通性辅助 ✓）⇒ 自造判据需先获授权 ✗。作者选 **(b)** ✓：
> **"这一层是否变得必须硬打"交由预注册试玩判定** ✓（协议里有"跑腿项"与"准备/撤回"两问 ✓），**本腿不新增可达性判据** ✓；后续若要做，按 (a) 单独立项（需先定义"可绕"：从入口到出口存在一条**不经过任何小队锚点 3 格内**的走法 ✓）。

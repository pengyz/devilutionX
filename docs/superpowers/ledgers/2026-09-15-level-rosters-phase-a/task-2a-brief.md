# Task 2a 简报（拆分自计划 Task 2；裁决 R13）

> **范围**：只做计划的**步骤 1-5**——`Source/tables/level_roster.{h,cpp}` 的**数据结构、TSV 解析与校验**，以及 `test/level_roster_test.cpp` 的全部**合成表驱动**用例（1 条通过路径 + 3 条拒绝路径）。**不写真实 TSV**（步骤 6 属 Task 2b），**不做生产接线**（Task 2b）。
> 需求来源仍是计划文档 `docs/superpowers/plans/2026-09-15-level-rosters-phase-a.md` 的 Task 2 小节（下方原文），但**步骤 6/7 不在本次范围**：本次提交信息用 `feat(roster): add the roster table types, loader and validation`。
> 本任务不依赖任何真实 TSV：所有用例自行构造 `std::vector<LevelRosterEntry>` / `std::vector<LevelRosterParams>` 驱动 `ValidateLevelRoster`。加载函数（`LoadLevelRoster`）本次**要写出来**（解析 + 校验 + 失败即 fatal），但**不写**任何加载它的用例（留给 Task 2b 用真实 TSV 验）。
> **重要**：`LoadLevelRoster()` 本次**不要**接到生产调用点上——接线属 Task 2b（否则在没有 TSV 时启动即 fatal）。

...
bool ChangeInvItem(Player &player, int slot, Size itemSize)
{
	...
	if (player.HoldItem._itype == ItemType::Gold) {
		GoldAutoPlace(player, player.HoldItem);
		...
		if (myPlayer.InvGrid[j] > 0) { // first slot of an item
			...
			if (myPlayer.InvList[ii]._iStackCount > 1) {
				DrawString(out, StrCat(myPlayer.InvList[ii]._iStackCount),
				    { position + Displacement { 2, InventorySlotSizeInPixels.height - 14 }, { InventorySlotSizeInPixels.width - 4, 14 } },
...
{
	player._pGold += goldStack._ivalue;
	goldStack._ivalue = 0;
	return true;
}

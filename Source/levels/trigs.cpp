...
bool ForceL1Trig()
{
	for (const uint16_t tileId : L1UpList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			if (currlevel > 1)
				FloatingInfoString = FormatRuntime(_("Up to level {:d}"), currlevel - 1);
			else
	...
	for (const uint16_t tileId : L1DownList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Down to level {:d}"), currlevel + 1);
			for (int j = 0; j < numtrigs; j++) {
...
bool ForceHiveTrig()
{
	for (const uint16_t tileId : L6UpList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Up to Nest level {:d}"), currlevel - 17);
			for (int j = 0; j < numtrigs; j++) {
				if (trigs[j]._tmsg == WM_DIABPREVLVL) {
					cursPosition = trigs[j].position;
					return true;
	...
	for (const uint16_t tileId : L6DownList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId
		    || dPiece[cursPosition.x + 1][cursPosition.y] == tileId
		    || dPiece[cursPosition.x + 2][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Down to level {:d}"), currlevel - 15);
			for (int j = 0; j < numtrigs; j++) {
				if (trigs[j]._tmsg == WM_DIABNEXTLVL) {
					cursPosition = trigs[j].position;
					return true;
...
bool ForceCryptTrig()
{
	for (const uint16_t tileId : L5UpList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Up to Crypt level {:d}"), currlevel - 21);
			for (int j = 0; j < numtrigs; j++) {
				if (trigs[j]._tmsg == WM_DIABPREVLVL) {
					cursPosition = trigs[j].position;
					return true;
	...
	for (const uint16_t tileId : L5DownList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Down to Crypt level {:d}"), currlevel - 19);
			for (int j = 0; j < numtrigs; j++) {
				if (trigs[j]._tmsg == WM_DIABNEXTLVL) {
					cursPosition = trigs[j].position;
					return true;
...
bool ForceSKingTrig()
{
	for (const uint16_t tileId : L1UpList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Back to Level {:d}"), Quests[Q_SKELKING]._qlevel);
...
bool ForceSChambTrig()
{
	for (const uint16_t tileId : L2DownList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Back to Level {:d}"), Quests[Q_SCHAMB]._qlevel);
			cursPosition = trigs[0].position;
...
bool ForcePWaterTrig()
{
	for (const uint16_t tileId : L3DownList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			FloatingInfoString = FormatRuntime(_("Back to Level {:d}"), Quests[Q_PWATER]._qlevel);
			cursPosition = trigs[0].position;

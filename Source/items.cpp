...
void PrintItemComparison(const Item &item)
{
	...
	AddInfoBoxString(FormatRuntime(_("Equipped: {:s}"), equipped->getName().str()));

	// Show equipped item's full stats for comparison
	if (equipped->_iClass == ICLASS_WEAPON) {
		if (equipped->_iMinDam == equipped->_iMaxDam)
			AddInfoBoxString(FormatRuntime(_("damage: {:d}"), equipped->_iMinDam));
		else
			AddInfoBoxString(FormatRuntime(_("damage: {:d}-{:d}"), equipped->_iMinDam, equipped->_iMaxDam));
	}
	if (equipped->_iClass == ICLASS_ARMOR) {
		AddInfoBoxString(FormatRuntime(_("armor: {:d}"), equipped->_iAC));
	}
